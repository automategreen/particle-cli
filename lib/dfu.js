...
var that = module.exports = {
	...
	// the Automate Green devices if necessary.

	_udevRulesDir: '/etc/udev/rules.d/',
	_udevRulesFile: '50-automategreen.rules',
	...
	_promptInstallUdevRules: function() {
		...
		if (that._udevRulesInstalled()) {
			console.log(chalk.bold.red(
				'Physically unplug and reconnect the Automate Green device and try again.'
			));
		...
		if (ans.install) {
			...
			child_process.exec(cmd, function(error, stdout, stderr) {
				if (error) {
					...
					console.log(chalk.bold.red(
						'Physically unplug and reconnect the Automate Green device.\n' +
						'Then run the automategreen command again.'
					));
